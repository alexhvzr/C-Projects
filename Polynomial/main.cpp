...
// A = +5x^7 -4x^3 +10x -2
// B = +3x^4 +1x^3
    cout << "Enter terms for polynomial A.  Enter a coefficient  then exponent for each term. Enter -1 -1 to terminate." << endl;
    cin >> A;                                     // or use a bunch of
   // setCoeff
    cout << "Enter terms for polynomial B.  Enter a coefficient then exponent for each term. Enter -1 -1 to terminate." << endl;
    cin >> B;                                     // or use a bunch of
  //  setCoeff
...
// outputs exactly what is in quotes: "B = +3x^4 +1x^3"
    ...
    coeff = D.getCoeff(-20000);              // handle value out of
    //range
